...
class HMM(object):
    ...
    def _viterbi(signal, trans, dists):
        ...
        '''
        nStates = np.shape(trans)[0] - 1
        hmm = HMM()
        
        #Get the most likely path for the signal ending in each state
        alphas, seq = hmm.getAlphaFromMax(signal, trans, dists, nStates)
        
        #Calculate the most likely ending state
        for i in range(0, nStates):
            alphas[i] = alphas[i] + np.log(trans[i, -1])
        ll = alphas[np.argmax(alphas)]
        seq = seq[np.argmax(alphas)]
        seq.append(np.argmax(alphas))
        del seq[0]
    ...
    def getAlphaFromMax(self, signal, trans, dists, nStates, start = True):
        alphas = []
        paths = []
        
        #an alpha for each state
        for i in range(0, nStates):
            prior = dists[i].loglik(signal[:, -1])
            
            #if(start == True):
                    #paths.append([])
                    #prevSig = np.delete(signal, -1, 1)
                    #alphasPrev, pathsPrev = self.getAlphaFromMax(prevSig, trans, dists, nStates, start = False)
                    #for j in range(0, nStates):
                       # alphasPrev[j] = alphasPrev[j] + np.log(trans[j, i])
                    #alphas.append(prior + alphasPrev[np.argmax(alphasPrev)])
                    #paths[i] = pathsPrev
            
            #else:
            #recursively calculate most likely routes
            if(np.shape(signal)[1] > 1):
                prevSig = np.delete(signal, -1, 1)
                alphasPrev, pathsPrev = self.getAlphaFromMax(prevSig, trans, dists, nStates, start = False)
                for j in range(0, nStates):
                    alphasPrev[j] = alphasPrev[j] + np.log(trans[j, i])
                alphas.append(prior + alphasPrev[np.argmax(alphasPrev)])
                paths.append([])
                paths[i] = pathsPrev[np.argmax(alphasPrev)]
                paths[i].append(np.argmax(alphasPrev))
        
            #exit the recursion at t = 0
            else:
                alphas.append(prior + np.log(trans[-1, i]))
                paths.append([])
                paths[i] = [i]
    ...
    def _score(signal, seq, trans, dists):
        ...
        #the likelyhood of the first state
        ll = np.log(trans[-1, seq[0]])
        
        #adding the likelyhood of each jump between the first and last state
        for n in range(0, len(seq)-1):
            priorProb = dists[seq[n]].loglik(signal[:,n])
            ll += priorProb + np.log(trans[seq[n], seq[n+1]])
            
        #adding the likelyhood of the last jump
        lastPriorProb = dists[seq[-1]].loglik(signal[:,-1])
        ll += lastPriorProb + np.log(trans[seq[-1], -1])
        
        return ll
    
    def getAlpha(self, signal, trans, dists, nStates):
        alphas = []
        
        #the alphas for the observation at the end of the given signal
        for j in range(0, nStates):
            prior = dists[j].loglik(signal[:,-1])
            
            #recursively calculate previous alphas
            if(np.shape(signal)[1] > 1):
                sigTminOne = np.delete(signal, -1, 1)
                toSum = []
                prevAlphas = self.getAlpha(sigTminOne, trans, dists, nStates)
                for i in range(0, nStates):
                    #if(prevAlphas[i] != -np.inf): #do not take 0 likelyhood into account
                    toSum.append(prevAlphas[i] + np.log(trans[i, j]))
                alphas.append(prior + sc.logsumexp(toSum))
                
            #exit condition for recursion
            else:
                alphas.append(prior + np.log(trans[-1, j]))
    ...
    def _forward(signal, trans, dists):
        ...
        '''
        #get the alphas of the last time instance of the signal
        hmm = HMM()
        nStates = np.shape(trans)[1] - 1
        alphasLast = hmm.getAlpha(signal, trans, dists, nStates)
        
        #multiply each final alpha with their corresponding transition prob to state n+1
        toSum = []
        for j in range(0, nStates):
            if(np.log(trans[j, -1]) != -np.inf):
                toSum.append(alphasLast[j] + np.log(trans[j, -1]))
                
        #check for chance of only one path to exit
        if(len(toSum) > 1):
            ll = sc.logsumexp(toSum)
        else: 
            ll = toSum[0]

        return ll
    
    def _countSiToSj(self, stateSequence, transCount, n_States):
        '''
        ...
        #take into account transition from state -1
        transCount[-1, int(stateSequence[0])] += 1
        
        #count transitions between states -1 and n_States+1
        for xn in range(0, len(stateSequence) - 1):
            transCount[int(stateSequence[xn]), int(stateSequence[xn + 1])] += 1
            
        #take into account transition to state n_States+1
        transCount[int(stateSequence[len(stateSequence) - 1]), n_States] += 1
    ...
    def _calcstates(self, trans, dists):
        ...
        #updating the state table and log likelyhood
        for sig in self.signals:
            statesToAdd, sigLikelyhood = HMM._viterbi(sig, trans, dists)
            stateSequences.append(statesToAdd)
            ll += sigLikelyhood 
            for state in range(0, len(statesToAdd)): 
                states[index, int(statesToAdd[state])] = 1
                index += 1
              
        #removing unused states
        toRemove = np.zeros([np.shape(states)[1],])
        for n in range(0, np.shape(states)[1]):
            if (np.sum(states[:, n]) == 0):
                toRemove[n] = 1
        states = np.delete(states, np.where(toRemove), 1)
        
        #updating the transission matrix
        nStates = np.shape(states)[1]
        trans = np.zeros([nStates + 1, nStates + 1])
        index = 0
        for sig in self.signals:
            trans = self._countSiToSj(stateSequences[index], trans, nStates)
            index = index + 1
        for row in range(0, np.shape(trans)[0]):
            trans[row, :] = trans[row, :]/np.sum(trans[row, :])

        return np.array(states,dtype = bool), trans, ll
    ...
    def _updatecovs(self, states):
        ...
        for state in range(0, n_states):
            stateObs = self.data[:, states[:, state]]
            N = np.shape(stateObs)[1]
            
            #calculating the means
            if(N == 0):
                means[state, :] = np.zeros([d,])
            else:
                means[state, :] = np.sum(stateObs, axis = 1)/N
            
            #calculating the covariances
            if(N == 0):
                covs[state, :] = np.identity(d)
            else:
                D = stateObs - np.reshape(means[state, :], [d, 1])
                covs[state, :] = (D.dot(D.T))/(N-1)
            if(self.diagcov):
                covs[state, :] = np.diag(np.diagonal(covs[state, :]))
            if(np.array_equal(covs[state, :], np.zeros([d, d]))):
                covs[state, :] = np.identity(d)
